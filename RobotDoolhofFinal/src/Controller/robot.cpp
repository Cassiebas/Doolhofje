...
void controller::robot::init(SDL_Renderer *renderer, maze *yourMaze) {
...
void controller::robot::draw(SDL_Renderer *renderer) {	
	SDL_Rect dest;
	dest.x = location.x;
	dest.y = location.y;
...
void controller::robot::turn(float degrees) {
...
void controller::robot::setDesiredDegrees() { //deze eerst
...
void controller::robot::drive(float speed) { //dan deze
	if (driving == true) { 
...
bool controller::robot::isTurning() {
	return turning;
}

void controller::robot::setCurrentRoom(void) {
	current_room = myMaze->getRoomByCoords(location);
}

void controller::robot::update(void) {
	if (!path_init) {
		thePath.init(myMaze);
		path_init = true;
	}
